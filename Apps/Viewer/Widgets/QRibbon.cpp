    ...
    , _(new QRibbonPrivate(this))
{
    ...
    // 隐藏下边线
    connect(
        &(_->animationHideBar), &QPropertyAnimation::finished, this, &QRibbon::onHideTabFinished);
    ...
    styleActionGroup->addAction(
        _styleMenu->addAction("红色", [&]() { setColor("rgb(183, 71, 42)"); }));
    styleActionGroup->addAction(
        _styleMenu->addAction("蓝色", [&]() { setColor("rgb(43, 87, 154)"); }));
    styleActionGroup->addAction(
        _styleMenu->addAction("绿色", [&]() { setColor("rgb(33,115,70)"); }));
...
void QRibbon::initialize(QMainWindow* window)
{
    ...
    for (auto i : menus) {
        ...
        menuBar->removeAction(
            i);   /// 从菜单栏移除原有菜单项，否则当鼠标单击菜单位置时。仍会弹出菜单项
        // addAction(i);
...
void QRibbon::mouseMoveEvent(QMouseEvent* evt)
{
    if (_->_pressed) {
        ...
        bool maxOrFull =
            _->_mainWindow->windowState() & (Qt::WindowFullScreen | Qt::WindowMaximized);
...
{
    _->_mainWindow->setWindowState(_->_mainWindow->windowState() &
                                           (Qt::WindowFullScreen | Qt::WindowMaximized)
                                       ? Qt::WindowActive
                                       : Qt::WindowActive | Qt::WindowMaximized);
}


void QRibbon::onHideTabFinished()
{
    ui->widgetBottomBar->setMinimumHeight(
        _->animationHideBar.direction() == QAbstractAnimation::Forward ? 0 : 1);
}
